...
### 2. 뽀모도로 타이머
- 집중 시간 / 휴식 시간 커스텀 설정 (기본 25분/5분)
- 시작 / 일시정지 / 초기화 컨트롤
- 타이머 상태에 따라 혜달이 표정 자동 변화 (집중 → 휴식)
- 오늘 완료한 뽀모도로 횟수 표시 (localStorage 저장)
- 완료 시 **Web Audio API '첨벙!' 물소리 알림음** 재생
...
### 6. 상태 공유
- 현재 상태를 **URL 파라미터로 인코딩** (기분, 스탯, 타이머 상태, 할 일 등)
- 공유 모달에서 링크 복사
